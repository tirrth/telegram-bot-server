...
class ReadCallback {
 public:
  ...
  ReadCallback(std::function<bool(char* buffer, size_t& size)> callback) : size{-1}, callback{std::move(callback)} {
  }
...
class HeaderCallback {
 public:
  ...
  // NOLINTNEXTLINE(google-explicit-constructor, hicpp-explicit-conversions)
  HeaderCallback(std::function<bool(std::string header)> callback) : callback(std::move(callback)) {
  }
...
class DebugCallback {
 public:
  ...
  // NOLINTNEXTLINE(google-explicit-constructor, hicpp-explicit-conversions)
  DebugCallback(std::function<void(InfoType type, std::string data)> callback) : callback(std::move(callback)) {
  }
