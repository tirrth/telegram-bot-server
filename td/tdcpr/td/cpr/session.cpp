...
void Session::Impl::SetMultipart(Multipart&& multipart) {
  ...
  for (const Part& part : multipart.parts) {
    ...
    if (part.is_buffer) {
      ...
      curl_formadd(&formpost, &lastptr, CURLFORM_COPYNAME, part.name.c_str(), CURLFORM_BUFFER, part.value.c_str(),
                   CURLFORM_BUFFERPTR, part.data, CURLFORM_BUFFERLENGTH, part.datalen, CURLFORM_END);
    } else {
