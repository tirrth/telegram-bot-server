...
long Timeout::Milliseconds() const {
  ...
  if (ms.count() > std::numeric_limits<long>::max()) {
    ...
    std::cout << "cpr::Timeout: timeout value overflow: " << std::to_string(ms.count()) << "ms. " << std::endl;
  }
  // No way around since curl uses a long here.
  // NOLINTNEXTLINE(google-runtime-int)
  if (ms.count() < std::numeric_limits<long>::min()) {
    ...
    std::cout << "cpr::Timeout: timeout value underflow: " << std::to_string(ms.count()) << "ms. " << std::endl;
  }
